...
// Backend-Auswahl
var backendPicker = Ti.UI.createPicker({
    type: Ti.UI.createPicker.DOCUMENTS,
    bottom: 30,
    width: 220,
    height: 40
});

var cpuOption = Ti.UI.createPickerRow({ title: 'CPU', selected: true });
var gpuOption = Ti.UI.createPickerRow({ title: 'GPU (Metal)' });
backendPicker.add(cpuOption);
backendPicker.add(gpuOption);
...
// Plus-Button (Menu für Foto, Audio)
var plusMenu = Ti.UI.createMenu({
    title: '+'
});
var takePhotoAction = Ti.UI.createAction({
    id: 'take_photo',
    title: 'Foto aufnehmen',
    image: null
});
var photoLibraryAction = Ti.UI.createAction({
    id: 'photo_library',
    title: 'Fotomediathek',
    image: null
});
var recordVoiceAction = Ti.UI.createAction({
    id: 'record_voice',
    title: 'Stimme aufnehmen',
    image: null
});

plusMenu.addAction(takePhotoAction);
plusMenu.addAction(photoLibraryAction);
plusMenu.addAction(recordVoiceAction);

var plusButton = Ti.UI.createButton({
    systemButton: Ti.UI.iOS.SystemMenu.MORE
});
plusButton.menu = plusMenu;
plusButton.addEventListener('click', function() {
    // Menü wird vom System angezeigt
});
...
backendPicker.addEventListener('change', function(e) {
    selectedBackend = (e.index === 1) ? 'gpu' : 'cpu';
    backendInfo.text = selectedBackend === 'gpu'
        ? 'Schnellere Inferenz via Metal'
...
// Plus-Button (Menu-Auswahl)
plusMenu.addEventListener('action', function(e) {
    if (e.action === 'take_photo') {
        openCamera();
    } else if (e.action === 'photo_library') {
        openPhotoLibrary();
    } else if (e.action === 'record_voice') {
...
function toggleVoiceRecording() {
    if (isRecording) {
        ...
        // Button ändern
        plusButton.systemButton = Ti.UI.iOS.SystemButton.STOP;
        plusButton.menu = null;
        ...
        plusButton.addEventListener('click', function() {
            ...
            recorder.release();
            isRecording = false;
            plusButton.systemButton = Ti.UI.iOS.SystemMenu.MORE;
